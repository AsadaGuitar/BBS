...
private[adaptors] trait UserThreadsTable {
   ...
   *  @param threadId Database column thread_id SqlType(varchar), Length(255,true)
  ...
  case class UserThreadsRow(id: Int, userId: String, threadId: String, isClose: Boolean = false, createAt: java.sql.Timestamp, closeAt: Option[java.sql.Timestamp] = None)
  /** GetResult implicit for fetching UserThreadsRow objects using plain SQL queries */
  implicit def GetResultUserThreadsRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Boolean], e3: GR[java.sql.Timestamp], e4: GR[Option[java.sql.Timestamp]]): GR[UserThreadsRow] = GR{
    prs => import prs._
    UserThreadsRow.tupled((<<[Int], <<[String], <<[String], <<[Boolean], <<[java.sql.Timestamp], <<?[java.sql.Timestamp]))
  }
  ...
  class UserThreads(_tableTag: Tag) extends profile.api.Table[UserThreadsRow](_tableTag, "user_threads") {
    ...
    def ? = ((Rep.Some(id), Rep.Some(userId), Rep.Some(threadId), Rep.Some(isClose), Rep.Some(createAt), closeAt)).shaped.<>({r=>import r._; _1.map(_=> UserThreadsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val threadsFk = foreignKey("user_threads_threads_id", threadId, Threads)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
    /** Foreign key referencing Users (database name user_threads_user_id) */
    lazy val usersFk = foreignKey("user_threads_user_id", userId, Users)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
